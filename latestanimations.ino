...
const int mw = WIDTH;        // Matrix width
const int mh = MATRIX_ROWS;  // Matrix height
...
// ------------------------------
const char* ssid = "Stanford";  // <<< Replace with your WiFi network name
...
const time_t END_TIME = 1739725200000 + 900000;  // Example: adjust to your event’s Unix time
...
struct Confetti {
    ...
    // Constructor
    Confetti(int startX, float startY) {
        x = startX;
        y = startY;
        gravity = 0.1;
        vx = ((float)random(3) - 1.0) / 2.0;  // Random horizontal velocity
        vy = ((float)random(4)) / 2.0;       // Random upward velocity
        color = matrix->Color(random(255), random(255), random(255));
    }

    // Update function to move confetti
    void update() {
        x += vx;
        y += vy;
        vy += gravity; // Apply gravity

        // Reset if it falls below a certain point
        if (y > mh) {  
            reset();
        }
    }

    // Reset confetti to the top with new velocity
    void reset() {
        x = random(mw);  // Random x position
        y = 0.0;             // Reset to top
        vx = ((float)random(3) - 1.0) / 2.0;  // Random horizontal velocity
        vy = ((float)random(4)) / 2.0;       // Random upward velocity
        color = matrix->Color(random(255), random(255), random(255));
...
void updateHearts() {
  for (int i = 0; i < 4; i++) {
    ...
    if (hearts[i].x >= forbiddenLeft && hearts[i].x <= forbiddenRight && hearts[i].y >= forbiddenTop && hearts[i].y <= forbiddenBottom) {
...
// ------------------------------
// Fire effect stuff: https://github.com/toggledbits/MatrixFireFast/blob/master/MatrixFireFast/MatrixFireFast.ino
// ------------------------------
...
const uint32_t fire_colors[] = {
    0x000000,  // Black
    0x100000,  // Very dark red
    0x300000,  // Dark red
    0x500000,  // Medium dark red
    0x700000,  // Medium red
    0x900000,  // Bright red
    0xA00000,  // Bright red
    0xC02000,  // Orange-red
    0xC03000,  // Orange
    0xC04000,  // Bright orange
    0xC05000,  // Orange-yellow
    0xC06000,  // Yellow-orange
    0xC08000,  // Yellow
    0x907060,  // Pale orange
    0x807080   // Grayish end color
};
...
unsigned long fire_time = 0; /* keep time */
const int fire_fps = 15;
void make_fire() {
  uint16_t i, j;
  // if (fire_time > millis()) return;
  // fire_time = millis() + (1000 / fire_fps);
  ...
  for (i = 0; i < mh; ++i) {
    for (j = 0; j < mw; ++j) {
      // if (fire_colors[pix[i][j]] != 0) {
      //   Serial.println("y: ");
      //   Serial.print(mh - i - 1);
      //   Serial.print(" x: ");
      //   Serial.print(j);
      //   Serial.print(" c: ");
      //   Serial.print(fire_colors[pix[i][j]]);
      // }
      if (fire_colors[pix[i][j]] != 0) {
        matrix->drawPixel(j, mh - i - 1, fire_colors[pix[i][j]]);
        matrix->drawPixel(j, i, fire_colors[pix[i][j]]);
      }
      // Serial.println(fire_colors[pix[i][j]]);
    }
  }
}
...
void drawCountdown() {
  ...
  int textWidth = 4 * digitCellWidth + 1 * colonCellWidth;
...
// Hacker pattern
uint16_t hacker_current_spot = mh - 1;
int default_offsets[35] = { 9, 6, 1, 12, 7, 13, 2, 4, 8, 11, 10, 5, 3, 0 };
int hacker_offsets[WIDTH];

void hacker() {
  for (int col = 0; col < mw; col++) {
    for (float row = 0; row < mh; row++) {
      uint8_t green = (((row - hacker_current_spot - hacker_offsets[col]) + 1) / mh) * 256;
      uint16_t pixelColor = matrix->Color(0, green, 0);
      matrix->drawPixel(col, row, pixelColor);
    }
  }

  hacker_current_spot = (hacker_current_spot + 1) % mh;
}
...
const int conf_count = 250;
Confetti conf[conf_count];
...
void setup() {
  ...
  // setup hacker
  for (int i = 0; i < mw; i++) {
    // int def = default_offsets[i % 35] * 2;
    // int num = random(9);
    // hacker_offsets[i] = num - 4;
    hacker_offsets[i] = random(28);
  }

  for (uint16_t i = 0; i < conf_count; ++i) {
    conf[i] = Confetti(random(mw), 0);
  }
}
...
void loop() {
  ...
  //   matrix->drawPixel(conf[i].x, (int) conf[i].y, conf[i].color);
  // }
